...
unicodeData.UnicodeData.forEach(entry => {
  ...
  if ((isInterestingCategory || isPriorityBlock) && isUsefulCharacter(char, name)) {
    glyphData.push({
...
fs.writeFileSync(mainDataFile, JSON.stringify({
  stats,
...
fs.writeFileSync(compactDataFile, JSON.stringify({
  stats,
  characters: glyphData
}));
...
console.log(`   Processed: ${processedCount.toLocaleString()} total characters`);
console.log(`   Glyph Party: v${packageJson.version}`);
console.log(`   Unicode: ${ucdPackageJson.version}`);

console.log(`\n📁 Generated files:`);
console.log(`   ${mainDataFile} (${Math.round(fs.statSync(mainDataFile).size / 1024)}KB)`);
console.log(`   ${compactDataFile} (${Math.round(fs.statSync(compactDataFile).size / 1024)}KB)`);
