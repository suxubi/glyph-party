...
class GlyphParty {
    ...
    async loadData() {
        try {
            ...
            console.log(`✨ Loaded ${this.characters.length.toLocaleString()} characters`);
        } catch (error) {
    ...
    bindEvents() {
        ...
        document.getElementById('copy-code').addEventListener('click', () => {
            this.copyToClipboard(`U+${this.currentModalChar.code}`, 'Unicode code copied!');
        });

        document.getElementById('copy-html').addEventListener('click', () => {
            this.copyToClipboard(`&#${this.currentModalChar.decimal};`, 'HTML entity copied!');
        });
    ...
    setupFilters() {
        ...
        const categories = [...new Set(this.characters.map(char => char.category))].sort();
        const blocks = [...new Set(this.characters.map(char => char.block))].sort();
    ...
    filterCharacters() {
        this.filteredCharacters = this.characters.filter(char => {
            ...
            if (this.currentSearch) {
                ...
                if (!matchesName && !matchesCode && !matchesChar) {
                    return false;
                }
            }
    ...
    showCharacterDetail(char) {
        ...
        document.getElementById('modal-category').textContent = this.getCategoryName(char.category);
    ...
    updateStats() {
        if (this.stats) {
            document.getElementById('total-count').textContent = this.stats.totalCharacters.toLocaleString();
            document.getElementById('category-count').textContent = this.stats.categories;

            // Update version numbers
            if (this.stats.glyphPartyVersion) {
                document.getElementById('glyph-party-version').textContent = `v${this.stats.glyphPartyVersion}`;
            }
            if (this.stats.unicodeVersion) {
                document.getElementById('unicode-version').textContent = `Unicode ${this.stats.unicodeVersion}`;
            }
        }
    }

    updateVisibleCount() {
        document.getElementById('visible-count').textContent = this.filteredCharacters.length.toLocaleString();
    }
